...
async function extractFromFiling(filingText, companyName, ticker, formType, sector) {
  ...
  // Only mark as verified if confidence >= 0.85 AND qualified
  ...
  return {eventType:'other',eventSummary:ha?ha.claim.slice(0,80):('['+form+'] '+companyName),verifiedFacts:Object.entries(facts||{}).filter(function(e){return e[1]!=null&&e[1]!==''}).map(function(e){return e[0]+': '+e[1]}),inferences:ha?[{text:ha.claim,confidence:ha.confidence||0.7}]:[],materialityScore:a.materialityScore||50,overlookedReasons:ha?[ha.reasoning]:[],riskFlags:a.riskFlags||[],confidence:verConf,qualified:qualified,hiddenAngle:ha,isRoutine:a.isRoutine,contradictions:a.contradictions||[],whatToWatch:a.whatToWatch||[],catalystAttentionScore:a.catalystAttentionScore||50,extractedFacts:facts,industryProfile:profile.name,verificationConfidence:verConf,capitalStructureComplexity:a.capitalStructureComplexity||'unknown',verificationStatus:verification};
}
